...
\begin{frame}{The Fuzzy Sphere} % % % % % % % % % % % % % % % % % % %
    ...
    As algebra: replacing $x^i \mapsto \frac{1}{\sqrt{j(j+1)}} \pi_{j}(J_i)$, $[J_i, J_k] = i \epsilon_{ijk} J_k$, $\lbtext{\acal_N} = End(V_j) = M_{N}(\CC)$, understanding $V_j= \CC^{N+1} = span\{|j, m\ket\}_{|m| \leq j}$ as irrep. of $SU(2)$; this follows from  \then \rtext{$[x^i, x^j] = \frac{1}{\sqrt{j(j+1)}} i \epsilon_{ijk} x_k$}, $\sum x_1^2 + x_2^2 + x_3^2 = 1$.%. Under the adjoint action, which makes sense since R J_3 R^{-1} is rotation under rotated axis
    % The normalizig factor guarantees xum xx_i^2 = 1

...
\begin{frame}{Definitions} % % % % % % % % % % % % % % % % % % %
    ...
    $\bullet$ \textbf{$SU(2)$-coherent states}: minimum uncertainty states $\lbtext{|\phi, \theta)_N} := R_{(\phi, \theta)}|j, -j\rangle \in V_j$ \iff\ states \lbtext{$\psi^N_{(\phi, \theta)}$} \rtext{\iff\ points of $S^2$ $SU(2)$-equivariantly}, with \rtext{$SU(2)$-invariant distance}.

...
\end{proposition}
\begin{proof}
Shouldn't be too complicated once the correct framing is achieved.... $|j, j\rangle = \begin{pmatrix} 1&0 \end{pmatrix}$
\end{proof}
...
\begin{proof}
Let $b \in \acal_1$ be arbitrary. Then,
\begin{align*}
    |\omega_{\vec x}(b) - \omega_{\vec y}(b)| &= |(\vec x - \vec y)\cdot \vec b| \\
        &\leq |\vec x - \vec y||\vec b|.
\end{align*}

Now, for $a \in \acal_1 = M_2(\CC)$, let $a_\pm = a_1 \pm i a_2$, so that $a = a_0 + a_+ \sigma_+ + a_- \sigma_- + a_3 \sigma_3$. Recall that $\pi_{1/2}(J_k) = \frac{\sigma_k}{2}$, $k = 1, \dots, 3$, hence
\begin{equation*}
    i[D_1, a] = 
    \begin{pmatrix}
    0 & a_+ & -a_+ & 0\\
    ...
    \end{pmatrix}
\end{equation*}
and so $i[D_1, a]$ is hermitian, whose characteristic polynomial is $\lambda^2(\lambda&2 -4|\vec a|^2)$. Therefore, its norm  is its maximum eigenvalue:  $||[D_1, a]|| = 2 |\vec a|$.

Therefore, if $a \in \acal_1$ hermitian with $\vec a$ parallel to $\vec x - \vec y$ such that $2|\vec a| = 1$, this element saturates the inequality.
\end{proof}

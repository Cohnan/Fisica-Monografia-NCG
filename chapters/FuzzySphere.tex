...
However, an obvious problem with this definition is that the representation spaces $\acal_N'$ are not closed under their natural multiplication, so they are not algebras. But, from the case $N = 1$ there is a suggestion on how to define a multiplication\cite{Madore}: requiring the radical of the algebra is zero, there are only two possibilities, and only one being noncommutative:
\begin{align}

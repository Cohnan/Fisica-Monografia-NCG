...
This spectral triple may be seen to come as a single term of the $SU(2)$-induced expansion of $\slashed D$ \eqref{}. The Dirac operators can also be written as
\begin{equation}
...
\begin{remark}
If $\acal$ is a unital $C^*$-algebra, and $D$ is an associated Dirac oprator, the supremum  $d_D(\omega, \omega')$, of $|\omega(a) - \omega'(a)|$ for $a \in \acal$ such that $||[D, a]|| \leq 1$, is always attained in hermitian $(a = a^*)$ elements.
\end{remark}

\begin{remark}
A state may be defined from its action on hermitian elements of the $C^*$-algebra $\acal$, since from there it can be uniquely extended to all of $\acal$: any $a \in \acal$ can be written as $a = \frac{a+a^*}{2} + \frac{a - a^*}{2} \in Herm. + Antiherm. = Herm. + i\, Herm$.
\end{remark}

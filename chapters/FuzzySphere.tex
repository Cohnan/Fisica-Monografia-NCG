...
One important property of these spectral triples is their equivariance under $SU(2)$, implying that $SU(2)$-acts by isometries on the (state) spaces as shown in \ref{theoInvDistance}. The \lbtext{equivariance of a spectral triple} under the group $SU(2)$ reduces to/is\ref{}\cite{} the commutation of the action of the action of both the Dirac operator and the algebra on the Hilbert space, with the action of the group. Hence, , we must understand how $SU(2)$ is acting on the distinct sets involved in our ....

MY DEFINITIONS:
\begin{definition}
$(A, H, D)$ spectral triple. Let $G$ act on $H$ and $A$. The spectral triple is $G$-equivariant if the distant function induced in the state space $S(A)$ is $G$-invariant.
\end{definition}

\begin{theorem}
$(A, H, D)$ spectral triple. Let $G$ act on $H$ unitary, and on $A$. Then, if $[D, G\cdot] = 0$ and the induced action on $A$ as operators on $H$ interwines with the action on $A$, then the spectral triple is $G$-equivariant.
\end{theorem}
...
Due to theorem \ref{}, although the full spectral triple seems to be a better approximation of the canonical spectral triple due to the properties of the spectrum of $\dcal_N$, we will study the distances using the irreducible spectral triple since it induces exactly the same notion of distance on each $\acal_N$. From now on we will stop writing the symbols $\pi_j$ unless confusion may arise.

\begin{remark}
If $\acal$ is a unital $C^*$-algebra, and $D$ is an associated Dirac operator, the supremum  $d_D(\omega, \omega')$, of $|\omega(a) - \omega'(a)|$ for $a \in \acal$ such that $||[D, a]|| \leq 1$, is always attained in hermitian $(a = a^*)$ elements \ref{Rieffel30,33}.
\end{remark}
...
Under the distance function defined by the previously introduced spectral triples, \rtext{$SU(2)$ acts, in fact, by isometries on each $\acal_N$}.
...
\begin{proof}
    ...
    \begin{equation}
        \rtext{||[D_N, a^g]|| = ||[D_N, a]||},
    \end{equation} since, then,
    \begin{align*}
    d_N(g_* \omega, g_* \omega') &= sup_{a \in \acal_N}\{ |\omega(a^g) - \omega'(a^g)|: ||[D_N, a \otimes 1_2]|| \leq 1 \} \\
        &= sup_{b \in \acal_N}\{ |\omega(b) - \omega'(b)|: ||[D_N, b \otimes 1_2]|| \leq 1 \} \\
        &= d_N(\omega, \omega'),
\end{align*}
since $b = a^g$ for fixed $g \in SU(2)$ sweeps all $\acal_N$ if $a$ does.

The action on $\acal_N \ni a$ as a an operator subalgebra on $H_N$, i.e. as elements of the type $a \otimes 1 \subset End(H_N)$, is  $u(a \otimes 1)u^* = a^g \otimes 1$, coinciding with the already defined action on $a \in \acal_N$. 

    
\end{proof} since from there it follows that

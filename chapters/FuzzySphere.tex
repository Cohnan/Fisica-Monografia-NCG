...
\lbtext{WHY DO WE WANT TO STUDY DISTANCE BETWEEN COHERENT STATES???} Make this very explicit.
...
\begin{equation}\ref{saturatingDiagonalADiscreteBasis}
...
\begin{lemma} \label{lemmaFormulaAuxiliaryDistanceAllN}
\begin{align}
    \rho_N(\theta) &= \psi^N_{(0, \theta)}(\hat a) - \psi^N_{(0, 0)}(\hat a) \\
    &   = \sum_{k = 1} \binom{2j}{j+m} (\sin \frac{\theta}{2})^{2n} (\cos \frac{\theta}{2})^{2(N-n)}  \sum_{k = 1}^n \frac{1}{\sqrt{k(N - k+1)}}
\end{align}
\end{lemma}
\begin{proof}
For a diagonal element $a$,  
\begin{equation*}
    \psi^N_{(0, \theta)}(a) = \sum_{k = 1} \binom{2j}{j+m} (\sin \frac{\theta}{2})^{2(j+m)} (\cos \frac{\theta}{2})^{2(j-m)} \omega_m(a),
\end{equation*}
hence $|\psi^N_{(0, \theta)}(a) - \psi^N_{(0, 0)}(a)|$ over the diagonal elements can be bounded by the claimed distance using the triangle inequality and the bound $|| \leq d_N(\omega_m, \omega_{-j}) = \sum_{k = 1}^n \frac{1}{\sqrt{k(N - k+1)}}$. Using the diagonal hermitian element $\hat a$ introduced in \eqref{} to saturate the distance between the $\omega_m$'s, we obtain the desired formula.
\end{proof}

\begin{lemma}\label{lemmaDerivativeAuxiliaryDistance}
For $N \in \NN$ fixed, $0 \leq \rho'_N(\theta) \leq 1$.
\end{lemma}
\begin{proof}
From the first formula in lemma \ref{lemmaFormulaAuxiliaryDistanceAllN} we easily deduce that $\rho_N'(\theta) = \frac{d}{d\theta} \psi^N_{(0, \theta)} (\hat a)$. Furthermore, from the $SU(2)$-equivariance of the identification of coherent states and points in $S^2$, on the diagonal elements it follows that 
\begin{equation}
    \rho_N'(\theta) 
...
\end{equation*}
recalling that $[E, \hat a]$ is the ladder operator on $V_j$, of unit norm. From expanding the formula $\rho_N'(\theta) = (0, \theta| [E, \hat a] |0, \theta)_N$ using the definition \ref{}, the lower bound $0$ then follows.
\end{proof}

The following lemma is last piece of our puzzle, but its proof requires some detailed analysis that can is shown in \cite{DAndrea2013}.
\begin{lemma}: For all $N$, the sequence $\rho_N(\theta)$ converges uniformly to $\theta$ as $N\to \infty$. 
\end{lemma}

From all of this, the following theorem has been proven.
\begin{theorem}
$$

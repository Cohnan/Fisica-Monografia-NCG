...
\begin{eqnsplit}\label{equationsExtraVariables69D2}
    c_{m,m'}(k) &:= \frac{\sqrt{k_m} + \sqrt{k_{m'}}}{2} 
        = \sqrt{2k} - 1 + \frac{3-m^2-m'^2}{2\sqrt{2}} \frac{1}{\sqrt{k}} - \frac{2+m^2+m'^2}{2}\frac{1}{k} + O(\frac{1}{k^{3/2}}) \\
    \rho_{m,m'}(k) &:= \frac{2 + \sqrt{k_m}\tilde \rho_m + \sqrt{k_{m'}}\tilde \rho_{m'}}{2 c_{m,m'}}
        = \sqrt{2}\frac{1}{\sqrt{k}} + \frac{2+m^2 + m'^2}{4}\frac{1}{k} + O(\frac{1}{k^{3/2}})\\
...
\begin{align}
    \cut{x^\pm}  \psi_m
        &= \begin{cases}
        \left(1 + \frac{9}{8} \frac{1}{\sqrt{k}} + \frac{137 \pm 32m + 32m^2}{64 \sqrt{2}} \frac{1}{k} + O(\frac{1}{k^{3/2}}) \right) \psi_{m \pm 1} & \text{if } -\Lambda \leq \pm m \leq \Lambda -1
        \\
        0 & \text{otherwise}
        \end{cases}\\
    [\cut{x^+}, \cut{x^-}]  \psi_m
        &= \begin{cases}
        \left( -\frac{m}{k}  + O(\frac{1}{k^{3/2}}) \right) \psi_{m} & \text{if } |m| \leq \Lambda -1
        \\
        \pm \left( \frac{1}{2} + \frac{9}{4\sqrt{2}} \frac{1}{\sqrt{k}} +  \frac{109-16\Lambda + 16\lambda^2}{32} \frac{1}{k} + O(\frac{1}{k^{3/2}}) \right) \psi_{m} & \text{if } m = \pm \Lambda 
        \end{cases}\\
    R^2  \psi_m
        &= \begin{cases}
        \left( 1+ \frac{9}{2\sqrt{2}} \frac{1}{\sqrt{k}} -\left(\frac{109}{16}+m^2\right) \frac{1}{k}  + O(\frac{1}{k^{3/2}}) \right) \psi_{m} & \text{if } |m| \leq \Lambda -1
        \\
        \pm \left( 1+ \frac{9}{4\sqrt{2}} \frac{1}{\sqrt{k}} + \frac{109-16\Lambda+16\lambda^2}{32} \frac{1}{k} + O(\frac{1}{k^{3/2}}) \right) \psi_{m} & \text{if } m = \pm \Lambda 
        \end{cases}
\end{align}
where $R^2 := \cut{x^1}^2 + \cut{x^1}^2 = \cut{x^+}\cut{x^-} + \cut{x^-}\cut{x^+}$.
\end{proposition}
\begin{proof}
...
\begin{align*}
    \langle \psi_{m-1} | x^- | \psi_m \rangle 
        ...
        &= \frac{K_{m-1,m}}{\sqrt{2}} e^{\rho_{m-1,m} + \frac{1}{4 c_{m-1,m}}}.
\end{align*}
...
\begin{proposition}\todo{the notation for $m$ is bad, but it is for me to remember that perhaps something should be said about how the powers of $m$ increase with the powers of $k$} Up to second order of powers in $\frac{1}{\sqrt{k}}$ the following relations hold:
\begin{align}\label{equationActionChiD2}
    ...
    [\chi^+, \chi^-]  \psi_m
        &= \begin{cases}
         -\frac{m}{k} \psi_{m} & \text{if } |m| \leq \Lambda -1
        \\
        \pm \left( \frac{1}{2} + \frac{\Lambda(\Lambda-1)}{k} \right) \psi_{m} & \text{if } m = \pm \Lambda 
        \end{cases}\\
    ...
    \mathcal R^2  \psi_m
        ...
        \end{cases}
\end{align}
where $\mathcal R^2 := (\chi^1)^2 + (\chi^2)^2 = \chi^+\chi^- + \chi^-\chi^+ = R^2/a^2$.
\end{proposition}
\begin{remark}
Notice that if the action \eqref{equationActionChiD2} were used exactly to define the operators $\chi^\pm$, then equations \eqref{equationCommutatorChiD2} and \eqref{equationRChiD2} would also be exact. This is thanks to the fact that, although the square root in \eqref{equationActionChiD2} is only valid up to the power $\frac{1}{k}$, i.e. it is only meaningful to say that it is equal to $ 1+\frac{m(m+1)}{2}$, when we use it to calculate the commutator and the radius it produces the factors exactly up to the power $\frac{1}{k}$.
\end{remark}

\begin{proposition}[Summary]
...
\begin{align}\label{equationOperatorFormulaCommutatorsChiD2}
    ...
    [\chi^+, \chi^-] &= - \frac{\cut L}{k} + \left[ 1 + \frac{\Lambda(\Lambda+1)}{k}\right](\tilde P_\Lambda - \tilde P_\Lambda),
\end{align}
...
\end{equation}
Equations \eqref{equationOperatorFormulaCommutatorsChiD2}$_3$ and \eqref{equationOperatorFormulaRChiD2} are only valid up to second order in powers of $\frac{1}{\sqrt{k}}$; however, if the formulas \eqref{equationActionChiD2} are used as exact definitions of the operators, then this theorem is satisfied exactly.
\end{proposition}

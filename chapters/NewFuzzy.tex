...
As illustrated in Figure \ref{fig:D2EigenvaluesEigenEnergiesHarmonicCutoff}, if $k$ is steep enough% small enough with respect to $k$, e.g. $\cut E \lesssim 8\sqrt{k}$, 
...
Indeed, in Theorem \ref{theoremEquivalent*IsomorphismALgebraSphericaleimphiD2} we will prove that %(since $O(2)$ doesn't affect the radial coordinate), for any compatible $k$, $\hcal_{\cut E}$ is isomorphic to $\hcal'_{\cut E} := span\{e^{i m \phi} \, : \, m \in \ZZ, \, m^2 \leq \cut E\}$ in an $O(2)$-equivariant way, and, hence, that
...
\begin{eqnsplit}
    ...
    &= 1 + \frac{9}{4\sqrt{2}} \frac{1}{\sqrt{k}} + \frac{137}{64} \frac{1}{k} + \frac{715}{256 \sqrt{2}} \frac{1}{k^{\frac{3}{2}}} + O\left(\frac{1}{k^2}\right).
\end{eqnsplit}
...
The normalization constant $a$ is simply the factor that extracts the $m$ independent terms in the series expansion of the action of $\cut{x^\pm}$ on each $\psi_m$. The factor $a$ rapidly converges to $1$ with increasing $k$, as shown in the Figure \ref{fig:asolo}.% graph \ref{fig:acomparados} shows the some approximate values of $a$ plotted besides the theoretical value of $a$.
\begin{figure}[h]
    \centering

...
\subsection{Very Important Facts} % Everything written here is also written somewhere else
    ...
    \item \rtext{Why is $\mathcal A_N = M_{N+1}(\CC)$?}: what Madore did was actually defining $A_N := \bigoplus_{l = 0, 1, \dots}^N V_l$ where $V_j$ is the irrep. of $SO(3)$ of integer spin $2l$ (of homogeneous polynomials in $x_1, x_2, x_3$ of degree $l$), since this is a \rtext{truncation of the commutative algebra $L^2(S^2) \cong \bigoplus_{l\in \NN} V_l$ at $N = 2j$}, but this truncation is not an algebra; the algebra structure is, by some perhaps desirable properties, then imposed to be that of $M_{N+1}(\CC)$ which comes basically from changing the algebra generators $x_k \mapsto \hat x_k = \frac{1}{\sqrt{j(j+1)}}\pi_j(J_k)$ ($[J_i, J_j] = i \epsilon J_k$ generate $\sut$) (i.e. basically changing the $x_k$ by the rotation operator of spinors of spin $j$ w.r.t. this axis) (this, in turn, changes the vector spaces generators $Y_{lm}$ by some fuzzy spherical harmonics $\hat Y_{lm}$).
    ...
    \item Equivariance of the spectral triple under $SU(2)$: I THINK, TODO the action of the $SU(2)$ group / rotations is an isometry of the Fuzzy Sphere.

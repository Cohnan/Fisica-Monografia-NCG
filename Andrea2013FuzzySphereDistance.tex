...
\subsection{Very Important Facts} % Everything written here is also written somewhere else
    ...
    \item \rtext{\Huge \textbf{Summary}: Start from $SU(2)$-equivariance (and  their corresponding ``equivariant'' decompositions) of the commutatative $\slashed D$ and $\mathcal H = L^2(S^2) \otimes \CC^2$; the desired: ``truncated spectral triple'' comes from truncating the decompositions at $N = 2j$.}

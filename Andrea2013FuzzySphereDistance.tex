...
\subsection{Very Important Facts} % Everything written here is also written somewhere else
    ...
    \item \rtext{What kind of spectral triple do we want?}: \rtext{find an ``\textbf{$SU(2)$-equivariant}'' espectral triple $\supset$ operator $D_N$ on $\mathcal A_N$ whose spectrum is that of $\slashed D: L^2(S^2) \otimes \Sigma_2 \to \slashed D: L^2(S^2) \otimes \Sigma_2$ \textbf{truncated} at $N = 2j$}. 

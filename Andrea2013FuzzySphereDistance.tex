...
\subsection{Very Important Facts} % Everything written here is also written somewhere else
    ...
    \item In summary, what is $SU(2)/su(2)$-equivariant?:
        ...
        \item The Dirac operators / The spectral triples
...
\subsection{Important Facts}
    ...
    \item The equivariance of the spectral triples comes from the commuting properties of the algebraic Dirac operators with the range of the coproduct of $U(\alg g)$, AND \dbtext{something else}. The first part makes sense since it means that, once we apply the representations $\pi_j \otimes \pi_{1/2}$ the corresponding (to a rotation and to the Dirac element) operators commute, \textbf{i.e. rotations and the Dirac operators commute.}
